...
// LIGHT-STYLE PASS: Molly Light's own version, independent of the upstream
// canonicalVersionCode/canonicalVersionName above. Every build this fork shipped
// before this had the exact same versionCode/versionName -- both were derived
// purely from the upstream fields above, which only change when a new upstream
// Molly release is imported, not when this fork's own reskin/settings/bugfix
// work changes. That meant update checkers (e.g. Obtainium) could never detect
// a new release. Bump mollyLightVersionMinor (or Major, for a bigger jump) each
// time a new build is released; that's the only version number that needs
// touching going forward.
//
// mollyLightBaselineVersionCode pins versionCode to what every previously-shipped
// build already used (derived from the values above, which have been constant
// for every real-device install so far), so this can never regress below what's
// already installed -- Android refuses same-package upgrades with a lower
// versionCode than what's currently installed.
val mollyLightBaselineVersionCode = (canonicalVersionCode * maxHotfixVersions) + mollyRevision + currentHotfixVersion
val mollyLightVersionMajor = 1
val mollyLightVersionMinor = 4
...
android {
  ...
  defaultConfig {
    val mollyLightVersionName = "$mollyLightVersionMajor.$mollyLightVersionMinor"

    versionCode = mollyLightBaselineVersionCode + (mollyLightVersionMajor * 10000) + mollyLightVersionMinor
    versionName = if (ciEnabled) requireCommitTag() else mollyLightVersionName
